...
double irram_sintan(double a, double b, double c)
{
	REAL a_real(a);
	REAL b_real(b);
	REAL c_real(c);
...
void compute() {
	...
	iRRAM::cin >> a_str >> b_str >> c_str;
	double a_double = binary2double(a_str);
	double b_double = binary2double(b_str);
	double c_double = binary2double(c_str);
	double r_double = irram_sintan(a_double, b_double, c_double);
