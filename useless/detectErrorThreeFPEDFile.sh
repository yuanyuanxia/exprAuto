...
# generate data
gcc geneDataThree.c -IincludeTEST -Wall -Wextra -Wpedantic
./a.out {x0Start} ${x0End} ${x1Start} ${x1End} ${x0Size} ${x1Size}


testFileName=test3paramFPEDFile

echo "Detecting error: ${uniqueLabel} ${x0Start} ${x0End} ${x1Start} ${x1End} ${x2Start} ${x2End} ${x0Size} ${x1Size} ${x2Size} ${prefix} ${middle} ${suffix}"

${CC} ${testFileName}.c ${prefix}_${suffix}.c ${prefix}_mpfr.c computeULP.c -IincludeTEST -DEXPRESSION=${prefix}_ -DSUFFIX=${suffix} -Wall -Wextra -Wpedantic -lmpfr -lm -o ${testFileName}.exe
