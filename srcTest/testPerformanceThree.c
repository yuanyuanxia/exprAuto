...
#ifndef FUNCNAME
#define FUNCNAME doppler1
#endif
#ifndef RUNTIME
#define RUNTIME 10000
#endif

double FUNCNAME(double, double, double);
...
int main(int argc, char *argv[])
{
    ...
    unsigned long sum;
    uint64_t tic, toc;
    double inputdataX0[RUNTIME], inputdataX1[RUNTIME], inputdataX2[RUNTIME];
    double result[RUNTIME], sumResult = 0, test = 1;
    double data_x0Start = 0, data_x0End = 1, data_x1Start = 0, data_x1End = 1, data_x2Start = 0, data_x2End = 1;
    FILE *stream_time;
    ...
    if(argc == 8)
    {
        ...
        data_x2Start = atof(argv[6]);
        data_x2End = atof(argv[7]);
    }
    ...
    for(i = 0; i < RUNTIME; i++)
    {
        ...
        inputdataX2[i] = (double)rand() / ((double)RAND_MAX + 1) * ((data_x2End) - (data_x2Start)) + (data_x2Start);
    }
    ...
    for(i = 0; i < 100; i++)
    {
        FUNCNAME(inputdataX0[i], inputdataX1[i], inputdataX2[i]);
    }
    for(i = 0; i < RUNTIME; i++)
    {
        tic = rdtscp();
        result[i] = FUNCNAME(inputdataX0[i], inputdataX1[i], inputdataX2[i]);
        toc = rdtscp();
