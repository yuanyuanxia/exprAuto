...
int expr_squareRoot3_2_mpfr_old(double x, mpfr_t mpfrResult) {
	int status = 0;
...
int expr_squareRoot3_1_mpfr(double x, mpfr_t mpfrResult) {
	int status = 0;
...
int expr_squareRoot3_2_mpfr(double x, mpfr_t mpfrResult) {
	if (x < 1e-5) {
		expr_squareRoot3_1_mpfr(x, mpfrResult);
	} else {
		expr_squareRoot3_2_mpfr_old(x, mpfrResult);
	}
}