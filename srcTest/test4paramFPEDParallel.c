...
struct errorInfo test4paramFPEDParallel(unsigned long int testNumX0, unsigned long int testNumX1, unsigned long int testNumX2, unsigned long int testNumX3, const char* uniqueLabel, const char* fileNameKernel, int myid, int i0StartLocal, int i0EndLocal, int i1StartLocal, int i1EndLocal, int i2StartLocal, int i2EndLocal, int i3StartLocal, int i3EndLocal, double x0startOriginInterval, double x1startOriginInterval, double x2startOriginInterval, double x3startOriginInterval, double stepX0, double stepX1, double stepX2, double stepX3) {
    DL maxInputX0, maxInputX1, maxInputX2, maxInputX3;
    int i0, i1, i2, i3;
    // int flag;
    double x0, x1, x2, x3, reUlp, sumError, aveReUlp, maxReUlp, lenX0, lenX1, lenX2, lenX3;
        ...
        for(i2 = i2StartLocal; i2 <= i2EndLocal; i2++) {
            ...
            for(i1 = i1StartLocal; i1 <= i1EndLocal; i1++) {
                ...
                for(i0 = i0StartLocal; i0 <= i0EndLocal; i0++) {
                    ...
                    computeResult4param(x0, x1, x2, x3, mpfrResult);
                    computeOrcle4param(x0, x1, x2, x3, mpfrOrcle);
                    #ifdef SINGLE
...
int main(int argc, char **argv) {
    ...
    // parameters init
    DL x0Start, x0End, x1Start, x1End, x2Start, x2End, x3Start, x3End;
    unsigned long int testNumX0, testNumX1, testNumX2, testNumX3;
    x0Start.d = 1;
    x0End.d = 2;
    x1Start.d = 1;
    x1End.d = 2;
    ...
    x3Start.d = 1;
    x3End.d = 2;
    int x0startNowIdx, x1startNowIdx, x2startNowIdx, x3startNowIdx;
    double x0startOriginInterval, x1startOriginInterval, x2startOriginInterval, x3startOriginInterval;
    double stepX0, stepX1, stepX2, stepX3;
    ...
    if(argc == 27) {
        ...
        testNumX0 = strtod(argv[9], NULL);
        testNumX1 = strtod(argv[10], NULL);
        testNumX2 = strtod(argv[11], NULL);
        testNumX3 = strtod(argv[12], NULL);
        x0startNowIdx = atoi(argv[13]);
        x1startNowIdx = atoi(argv[14]);
        x2startNowIdx = atoi(argv[15]);
        x3startNowIdx = atoi(argv[16]);
        x0startOriginInterval = strtod(argv[17], NULL);
        x1startOriginInterval = strtod(argv[18], NULL);
        x2startOriginInterval = strtod(argv[19], NULL);
        x3startOriginInterval = strtod(argv[20], NULL);
        stepX0 = strtod(argv[21], NULL);
        stepX1 = strtod(argv[22], NULL);
        stepX2 = strtod(argv[23], NULL);
        stepX3 = strtod(argv[24], NULL);
        strcpy(fileNameKernel, argv[25]);
        strcpy(uniqueLabel, argv[26]);
    // } else if(argc == 18) {
    //     x0Start.d = strtod(argv[1], NULL);
    //     x0End.d = strtod(argv[2], NULL);
    //     x1Start.d = strtod(argv[3], NULL);
    ...
    int lenX3Local = testNumX3 / numProcs;
    int i3StartLocal;
    i3StartLocal = x3startNowIdx + myid * lenX3Local;
    int i3EndLocal;
    if(myid != numProcs - 1) {
        i3EndLocal = x3startNowIdx + (myid + 1) * lenX3Local - 1;
    } else {
        i3EndLocal = x3startNowIdx + testNumX3;
    }
    ...
    struct errorInfo err = test4paramFPEDParallel(testNumX0, testNumX1, testNumX2, testNumX3, uniqueLabel, fileNameKernel, myid, i0StartLocal, i0EndLocal, i1StartLocal, i1EndLocal, i2StartLocal, i2EndLocal, i3StartLocal, i3EndLocal, x0startOriginInterval, x1startOriginInterval, x2startOriginInterval, x3startOriginInterval, stepX0, stepX1, stepX2, stepX3);
    ...
    if(myid == 0)
    {
        ...
        aveError = aveError / (testNumX0 * testNumX1 * testNumX2 * testNumX3);
        DL maxInputX0, maxInputX1, maxInputX2, maxInputX3;
        ...
        fprintf(fErr, "\naveReUlp = %lg\nmaxInputX0 = 0x%016lx %lg, maxInputX1 = 0x%016lx %lg, maxInputX2 = 0x%016lx %lg, maxInputX3 = 0x%016lx %lg, maxReUlp = %lg\n", aveError, maxInputX0.l, maxInputX0.d, maxInputX1.l, maxInputX1.d, maxInputX2.l, maxInputX2.d, maxInputX3.l, maxInputX3.d, maxError);
